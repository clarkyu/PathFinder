...
首次启动用三屏对话定向（考生/家长 × 出分前/已出分/已录取），之后全应用围绕三个标签展开：

| 标签 | 回答的问题 | 内容 | 对应报告方法论 |
|---|---|---|---|
| ⛵ **此刻** | 现在该做什么 | 阶段时间线（可点击切换）、**下一步推荐引擎**（按阶段 × 身份 × 数据状态给 1 主 + 2 次行动卡）、本阶段清单与进度、三个关键调整信号 | 第四章行动建议、Recommendations 1–8 |
| 🧭 **罗盘** | 我是谁 | 48 题霍兰德 RIASEC 自评（雷达图 + 三字码 + 一致性分析 + 专业建议）、心流线索墙、成就动机模式、奥德赛三种五年、访谈收获语录；外链官方免费测评与科学性分级 | Holland RIASEC、心流、SIMA、人生设计课、informational interview |
| 🗺️ **航海图** | 我去哪里 | 方向池（候选院校+专业）、决策平衡单（六维加权打分 + 综合排序）、冲·稳·保梯度归档与填报避坑提醒 | 金树人平衡单、阳光志愿冲稳保、教育部市场预警 |

所有输入走**底部浮层、对话式提问**（"什么事让你忘记时间？"），每条记录沉淀到罗盘/航海图。右上角入口收纳：家长专区（自主支持型教养）、知识库（22 张理论卡片）、数据导出/导入、关于与声明。旧版链接（#/assess 等）自动重定向，数据完全兼容。
